...
### Data Categories: Interaction vs. Content

To build a complete profile of what users want to hear next, Spotify’s engines split data into two fundamental categories: **interaction data** (what users do) and **metadata** (what the content is). These systems ingest, process, and weigh these data types differently based on whether they are running collaborative or content-based algorithms.

#### Interaction Data (User-Behavioral Inputs)

Interaction data feeds **collaborative filtering** models. It tells the system how users interact with content and is split into explicit and implicit signals.

**Explicit Feedback** is direct, intentional input from users. It provides a clean signal of user sentiment but suffers from data scarcity because most users browse passively.

- **Likes / Thumbs Up:** Binary signals that heavily boost an item’s weight in a user’s embedding vector.
- **Playlist Additions:** One of the strongest explicit signals. Adding a song to a playlist indicates high intent to replay and provides context based on the playlist’s title (e.g., "Gym 2026").
- **Shares / Saves:** High-value explicit interactions that indicate a user finds the content valuable enough to bookmark or distribute.

**Implicit Feedback** is passive behavioral data collected automatically. It is massive in volume but noisy, requiring statistical inference to understand true user intent.

- **Skips:** A critical negative signal. Platforms track *when* the skip occurred. A skip within the first 5 seconds indicates strong dislike, whereas a skip at the 90% mark is often treated as a functional complete.
- **Completion Rate / Listen Time:** Continuous numerical data (e.g., fraction of track completed). For Spotify, total listen time is a primary optimization target.
- **Dwell Time:** How long a user lingers on a preview, search result, or playlist before scrolling past or clicking.

#### Content Data (Item-Attribute Inputs)

Content data feeds **content-based filtering** models. It maps the DNA of an asset so the system can find mathematical equivalents, solving the "cold start" problem for newly uploaded tracks.

**Acoustic / Audio Features** are extracted using digital signal processing to analyze raw audio files:

- **Tempo:** Measured in Beats Per Minute (BPM). Helps algorithms match the physical energy of a user’s current activity.
- **Key and Mode:** The musical scale (e.g., C Major, G Minor), used to ensure smooth, harmonically consistent transitions in auto-generated playlists.
- **Sonance Attributes:** Continuous values calibrated between 0.0 and 1.0 that define the track’s texture, including **acousticness** (probability the song uses non-electric instruments), **danceability** (tempo and rhythm stability), and **instrumentalness** (likelihood of no vocals).

**Semantic & Textual Metadata** translates human language and culture into machine-readable strings and vectors:

- **Mood & Genre Tags:** Categorical tags (e.g., "Chill," "Melancholic," "Lo-Fi") generated by a mix of human curators, publisher tags, and Natural Language Processing models scraping web articles or music reviews.
- **Lyrics Analysis:** NLP models run sentiment analysis on lyrics to extract emotional themes, reinforcing the structural mood tags.

#### Data Type Matrix

The following table outlines how these data types are structurally represented and utilized within Spotify’s recommendation pipeline:

| Data Type | Structural Representation | Primary Machine Learning Use |
| --- | --- | --- |
| **Explicit (Likes, Saves, Playlist Adds)** | Sparse Binary Matrices | Matrix Factorization, Collaborative Filtering |
| **Implicit (Skips, Listen Time)** | Continuous Floats, Time-Series Logs | Deep Ranking, Reward Signals for Contextual Bandits |
| **Acoustic (Tempo, Key, Valence, Danceability)** | Dense Numerical Vectors | Convolutional Neural Networks (CNNs) for audio profiling |
| **Textual (Mood, Lyrics, Tags)** | Tokenized Strings, Text Embeddings | NLP models, Vector Search (Approximate Nearest Neighbors) |
